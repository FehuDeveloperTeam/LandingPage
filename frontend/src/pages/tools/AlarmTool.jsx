...
const AlarmaTool = () => {
  ...
  // --- TEMPORIZADOR (Corregido y Funcional) ---
  const [timerSeconds, setTimerSeconds] = useState(0);
  const [timerActive, setTimerActive] = useState(false);
  const [timerInput, setTimerInput] = useState({ h: 0, m: 0, s: 0 });
  const timerRef = useRef(null);

  // --- NUEVA ALARMA (Con Label) ---
  const [newAlarm, setNewAlarm] = useState({ time: '08:00', label: '', days: [0,1,2,3,4,5,6] });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      alarmas.forEach(a => {
        if (a.enabled && a.time === currentTime && now.getSeconds() === 0 && !ringingAlarm) {
          triggerAlarm(a);
        }
      });
    }, 1000);
  ...
  useEffect(() => {
    if (timerActive && timerSeconds > 0) {
      ...
      triggerAlarm({ label: 'Temporizador', time: '00:00' });
  ...
  const formatChrono = (ms) => {
    ...
    return `${m.toString().padStart(2, '0')}:${(s % 60).toString().padStart(2, '0')}.${((ms % 1000) / 10).toFixed(0).toString().padStart(2, '0')}`;
  };
        ...
        <div className="max-w-6xl mx-auto px-4 py-8 space-y-12">
          ...
          <div className="flex justify-between items-center">
            <Link to="/herramientas" className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-blue-600 transition-colors">
              <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Volver a Herramientas
            </Link>
          </div>
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-gray-100 dark:border-white/5 pb-10">
            <div>
              ...
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] mt-4">Fehu Engineering Precision</p>
            </div>
          ...
          <main className="min-h-[400px]">
            {activeTab === 'alarmas' && (
              <div className="grid md:grid-cols-2 gap-6">
                <button 
                  ...
                  className="p-10 rounded-[2.5rem] border-2 border-dashed border-gray-200 dark:border-white/10 flex flex-col items-center justify-center gap-4 hover:border-blue-600 transition-all"
                >
                ...
                {alarmas.map(a => (
                  <div key={a.id} className="p-8 rounded-[2.5rem] bg-white dark:bg-white/5 border border-gray-100 dark:border-white/5 flex items-center justify-between">
                    <div>
                      <p className="text-4xl font-black italic tracking-tighter">{a.time}</p>
            ...
            {activeTab === 'cronometro' && (
              <div className="flex flex-col items-center py-10 space-y-10">
                <div className="text-8xl md:text-[10rem] font-black italic tracking-tighter text-blue-600">
                  {formatChrono(chronoTime)}
                </div>
                <div className="flex gap-4">
                  <button onClick={() => setChronoRunning(!chronoRunning)} className="h-20 w-20 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-xl shadow-blue-600/20">
                    {chronoRunning ? <Pause size={30} /> : <Play size={30} fill="white" />}
                  </button>
                  <button onClick={() => {setChronoTime(0); setChronoRunning(false)}} className="h-20 w-20 rounded-full border border-gray-200 dark:border-white/10 flex items-center justify-center"><RotateCcw size={30}/></button>
                </div>
              </div>
            )}
            ...
            {activeTab === 'temporizador' && (
              <div className="flex flex-col items-center py-10 space-y-12">
                {!timerActive ? (
                  <div className="flex gap-4">
                    {['h', 'm', 's'].map(u => (
                      <input key={u} type="number" placeholder={u.toUpperCase()} className="w-24 p-6 text-4xl font-black text-center bg-gray-100 dark:bg-white/5 rounded-2xl outline-none focus:ring-2 ring-blue-600 transition-all"
                ...
                <button onClick={timerActive ? () => setTimerActive(false) : startTimer} className={`px-12 py-5 rounded-2xl font-black uppercase tracking-widest ${timerActive ? 'bg-red-500' : 'bg-blue-600'} text-white shadow-xl`}>
          ...
          {showModal && (
            <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-white/80 dark:bg-black/80 backdrop-blur-md">
              <div className="bg-white dark:bg-[#111] w-full max-w-sm rounded-[3rem] p-10 border border-gray-200 dark:border-white/10 shadow-2xl space-y-6">
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-black uppercase italic">Configurar</h3>
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-xl p-4 text-center font-bold outline-none"
                ...
                <button 
                  onClick={() => {
                    ...
                    setShowModal(false);
                    setNewAlarm({ time: '08:00', label: '', days: [0,1,2,3,4,5,6] });
                  }}
                  className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase tracking-widest"
                >
                  Guardar Alarma
                </button>
              </div>
          ...
          {ringingAlarm && (
            <div className="fixed inset-0 z-[1000] bg-blue-600 flex flex-col items-center justify-center p-10 text-center text-white">
              <Bell size={80} className="animate-bounce mb-8" />
              <h2 className="text-7xl md:text-9xl font-black italic tracking-tighter mb-4">{ringingAlarm.time}</h2>
              <p className="text-2xl font-black uppercase tracking-[0.2em] mb-12">{ringingAlarm.label || 'ALERTA'}</p>
              <button onClick={stopAlarm} className="w-full max-w-xs py-6 bg-white text-blue-600 rounded-3xl font-black uppercase tracking-widest shadow-2xl">Detener</button>
            </div>
          )}
